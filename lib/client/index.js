...
const safePathJoin = (str) => str.replace(/(^\/|\/$)/g, '');
...
class Firstclasspostcodes {
  ...
  async request(params = {}) {
    ...
    const { host, protocol, basePath, content } = this.config;

    const uri = [host, basePath, path].filter(Boolean).map(safePathJoin).filter(Boolean).join('/');
